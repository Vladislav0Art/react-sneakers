...
const Drawer = ({ currentSum, removeItem, onClose }) => {
  const { cartItems, setCartItems, theme } = React.useContext(AppContext);
  ...
  return (
    <div className="drawer">
      ...
      <div className={`d-flex flex-column justify-between drawer__sidebar ${theme}`}>

        <div>
          <div className="d-flex justify-between align-center mb-30 mt-30">
            <h2>Корзина</h2>

            <button onClick={onClose} className={`d-flex justify-center align-center drawer__btn main-btn ${theme}`}>
        ...
        {
          (cartItems.length > 0 && !isOrderCreated) && 
          <ul className={`drawer__order mb-30 ${theme}`}>
            ...
            <li className="d-flex justify-center">
              <button disabled={isLoading} onClick={onClickOrder} className={`drawer__actionBtn action-btn ${theme}`}>
